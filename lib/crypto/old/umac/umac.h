...
static uhash_ctx_t uhash_alloc(char key[16]);
...
static int uhash_reset(uhash_ctx_t ctx);

static int uhash_update(uhash_ctx_t ctx,
               char       *input,
               long        len);

static int uhash_final(uhash_ctx_t ctx,
              char        ouput[]);



struct uhash_state umac_state ;
