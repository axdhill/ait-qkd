...
Here are the steps which help you to setup a build system capable of compiling the sources on a pure Debian Wheezy/Jessie system.
    ...
    $ sudo apt-get install build-essential g++ gcc libboost-all-dev libssl-dev uuid-dev cmake libssl-dev libgmp3-dev libzmq3-dev libdbus-1-dev libqt4-dev libqwt-dev doxygen texlive-latex-base texlive-latex-extra texlive-font-utils dbus-x11 libcap2-bin python3
...
This will create a package `qkd_9.9999.6_amd64.deb` (or with the current version number embedded) in the current build folder.
...
If you have built the debian package on the target system then a
    ...
    $ sudo dpkg --install qkd_9.9999.6_amd64.deb 
...
However, if you lack certain packages and get error messages like these
    ...
    Unpacking qkd (from qkd_9.9999.6_amd64.deb) ...
