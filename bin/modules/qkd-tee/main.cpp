...
int main(int argc, char ** argv) {
    ...
    if (cVariableMap.count("bob")) {
        cQKDTee.set_role((unsigned long)qkd::module::module_role::ROLE_BOB);
    }
    else {
        cQKDTee.set_role((unsigned long)qkd::module::module_role::ROLE_ALICE);
    }
    if (cVariableMap.count("dbus")) {
        std::cout << cQKDTee.service_name().toStdString() << std::endl;
    }
    if (cVariableMap.count("config")) {
        cQKDTee.configure(QString::fromStdString(cVariableMap["config"].as<std::string>()), true);
    }
    ...
    if (cVariableMap.count("file") == 1) {
        ...
        cQKDTee.set_file_url(QString("file://") + QString::fromStdString(boost::filesystem::absolute(cFile).string()));
    }
    if (cVariableMap.count("run")) cQKDTee.start_later();
