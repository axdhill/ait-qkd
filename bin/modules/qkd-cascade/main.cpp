 ...
 * main.cpp
 * 
 * This is the CASCADE STANDARD QKD Module.
 * 
...
int main(int argc, char ** argv) {
    ...
    std::string sApplication = std::string("qkd-cascade - AIT QKD Module 'cascade' V") + qkd::version();
    ...
    cOptions.add_options()("passes,p", boost::program_options::value<uint64_t>()->default_value(14), "number of cascade passes");
    ...
    qkd_cascade cQKDCascade;
    cQKDCascade.set_debug_message_flow(cVariableMap.count("debug-message-flow") > 0);
    cQKDCascade.set_debug_key_sync(cVariableMap.count("debug-key-sync") > 0);
    if (cVariableMap.count("bob")) {
        cQKDCascade.set_role((unsigned long)qkd::module::module_role::ROLE_BOB);
    }
    else {
        cQKDCascade.set_role((unsigned long)qkd::module::module_role::ROLE_ALICE);
    }
    if (cVariableMap.count("config")) {
        cQKDCascade.configure(QString::fromStdString(cVariableMap["config"].as<std::string>()), true);
    }
    if (cVariableMap.count("run")) cQKDCascade.start_later();
    
    cApp.connect(&cQKDCascade, SIGNAL(terminated()), SLOT(quit()));
