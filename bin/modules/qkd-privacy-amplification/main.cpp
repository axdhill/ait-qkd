 ...
 * main.cpp
 * 
 * This is the PRIVACY AMPLIFICATION QKD Module.
 * 
 * This QKD Module runs the AUTHENTICATION protocol
 * 
...
int main(int argc, char ** argv) {
    ...
    qkd_privacy_amplification cQKDPrivacyAmplification;
    cQKDPrivacyAmplification.set_debug_message_flow(cVariableMap.count("debug-message-flow") > 0);
    cQKDPrivacyAmplification.set_debug_key_sync(cVariableMap.count("debug-key-sync") > 0);
    if (cVariableMap.count("bob")) {
        cQKDPrivacyAmplification.set_role((unsigned long)qkd::module::module_role::ROLE_BOB);
    }
    else {
        cQKDPrivacyAmplification.set_role((unsigned long)qkd::module::module_role::ROLE_ALICE);
    }
    if (cVariableMap.count("config")) {
        cQKDPrivacyAmplification.configure(QString::fromStdString(cVariableMap["config"].as<std::string>()), true);
    }
    if (cVariableMap.count("run")) cQKDPrivacyAmplification.start_later();

    cApp.connect(&cQKDPrivacyAmplification, SIGNAL(terminated()), SLOT(quit()));
    int nAppExit = cApp.exec();
    cQKDPrivacyAmplification.join();
