...
int main(int argc, char ** argv) {
    ...
    std::string sApplication = std::string("qkd-cat - AIT QKD Module 'cat' V") + VERSION;
    ...
    if (cVariableMap.count("bob")) {
        cQKDCat.set_role((unsigned long)qkd::module::module_role::ROLE_BOB);
    }
    else {
        cQKDCat.set_role((unsigned long)qkd::module::module_role::ROLE_ALICE);
    }
    if (cVariableMap.count("dbus")) {
        std::cout << cQKDCat.service_name().toStdString() << std::endl;
    }
    if (cVariableMap.count("config")) {
        cQKDCat.configure(QString::fromStdString(cVariableMap["config"].as<std::string>()), true);
    }
    if (cVariableMap.count("run")) cQKDCat.start_later();
    ...
    if (cVariableMap.count("file") == 1) {
        ...
        cQKDCat.set_file_url(QString("file://") + QString::fromStdString(boost::filesystem::canonical(cFile).string()));
    }
    if (cVariableMap.count("loop") == 1) cQKDCat.set_loop(true);
    
    cApp.connect(&cQKDCat, SIGNAL(terminated()), SLOT(quit()));
